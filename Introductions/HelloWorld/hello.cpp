...
int main(int argc, char* argv[]) { // alternate parameter: char** argv
	...
	/*
	The :: symbol is the scope-resolution operator; it is used to access static class members,
	or variables/methods/declarations from a "namespace", the equivalent of Java's packages.

	cout is a variable (NOT A METHOD) in the "std" package, which is why we have scope it to use cout, as in
	std::cout << ...
	This can be tedious, so you can "use" a namespace to bring all its definitions into the main program scope.
	*/
