...
int main(int argc, char* argv[]) {
	...
	// bool, char, short, int, long, float, double, long long, long double
	...
	// bools are actually integers and can be assigned integer values. Any non-zero integer is treated as "true".
	b = 2; // what is b?
	...
	// if you involve a bool in arithmetic, it is treated as having a value of 1 if true, and 0 if false.
	x = x + b;
	...
	// C++ DOES NOT GUARANTEE the # of bytes for most fundamental types. it only makes the following promise:

	// sizeOfChar = 1
	// sizeOfShort <= sizeOfInt <= sizeOfLong <= sizeOfLongLong

	// the actual sizes are IMPLEMENTATION-DEPENDENT (whoever wrote the compiler gets to choose)
